...
test("discards whitespace", () => {
    expect(tokenize("a+b", F).map(t => [t.type, t.text])).toEqual(tokenize("    a   \n +      b\n        ", F).map(t => [t.type, t.text]));
});
...
test("parses names", () => {
    expect(tokenize("a b coffee", F).map(t => t.text)).toEqual(["a", "b", "coffee"]);
});
test("maintains column and line", () => {
    expect(tokenize("a\nb c", F).map(t => t.location)).toEqual([new LocationTrace(0, 0, F), new LocationTrace(1, 0, F), new LocationTrace(1, 2, F)]);
});
test("parses string with escapes", () => {
    expect(tokenize('"abc\\xFF123"', F).map(t => t.type)).toContain(TokenType.STRING_ESC);
});
test("parses unicode escapes", () => {
    expect(tokenize('"\\u{1F914}"', F).map(t => [t.type, t.text])).toEqual([
